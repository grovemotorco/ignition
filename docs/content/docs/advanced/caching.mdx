...
```bash
ignition check deploy.ts @web -i hosts.ts --cache
```
...
- **Repeated checks** — running `ignition check` multiple times while iterating on a recipe
- **Large inventories** — checking 50+ hosts where most resources are already in desired state
- **CI pipelines** — speed up repeated `ignition check` runs in the same pipeline

## Clearing the cache

```bash
# Clear before running
ignition check deploy.ts @web --cache --cache-clear
