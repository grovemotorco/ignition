...
| `check`      | Inspects current state and reports whether changes are needed. It must be side-effect-free. |
| `apply`      | Mutating. Converges to the desired state and returns the output.                            |

## Implementing check()

`check()` must be side-effect-free. It inspects the remote host and returns a `CheckResult` that is
safe to execute during `ignition run --check`:
...
- Keep mutating or user-controlled preconditions out of `check()` so `ignition run --check` stays trustworthy
- If the final apply decision depends on an imperative precondition, return `inDesiredState: false`
  conservatively from `check()` and evaluate that precondition inside `apply()`
