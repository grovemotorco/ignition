...
```bash
ignition run deploy.ts <targets...> -i hosts.ts
```
...
| Multiple  | `web-1 web-2 @db`     | Space-separated mix             |
...
ignition run deploy.ts @web admin@10.0.0.5 -i hosts.ts
```

## Inspecting the inventory

List hosts in an inventory file:

```bash
ignition inventory hosts.ts
```

This shows all hosts with their resolved connection details and variables.
...
```bash
ignition run deploy-web.ts @web -i hosts.ts
ignition run deploy-db.ts @db -i hosts.ts
ignition run deploy.ts @web @app @db -i hosts.ts
```
