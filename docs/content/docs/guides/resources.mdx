...
| `sensitivePaths` | `string[]` | Fields to redact in logs                     |
...
1. **`check()`** — read-only, no side effects. Returns whether the resource is in the desired state.
2. **`apply()`** — mutating. Only called if `check()` returned `inDesiredState: false` and the mode is `apply`.

This gives you:

- **Free dry-runs** — `ignition run --check` runs `check()` for every resource without calling `apply()`
