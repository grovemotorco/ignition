...
Resources execute sequentially in the order you write them. This is by design — provisioning steps often depend on previous ones (install a package before configuring it, write a config file before restarting the service).
...
Filter recipes by tag with `--tags`:

```bash
ignition run deploy.ts @web --inventory hosts.ts --tags web,deploy
```

When `--tags` is set, the recipe only runs if its tags overlap with the selected tags.
