...
export default async function (ctx: ExecutionContext) {
  const { apt, docker, exec, file, directory, service } = createResources(ctx)

  await apt({ name: "nginx", state: "present" })
  await docker({ name: "edge-cache", image: "redis:7", state: "started" })
  await service({ name: "nginx", state: "started" })
}
```

The `createResources(ctx)` factory returns all six built-in resources bound to the current context. Destructure the ones you need.
