...
- In check mode (`ignition check`), unguarded exec reports as "would change" since it's imperative.
...
- Command output (stdout/stderr) can be streamed in real time when verbose mode is enabled.
