...
- **Known lint issue**: `bun run lint` (oxlint with `--type-aware --type-check`) reports TS2307 errors in `examples/file-roundtrip.ts` for the self-referencing `@grovemotorco/ignition` import. This is pre-existing on `main` and does not block `bun run typecheck` (tsgo), which passes cleanly.
